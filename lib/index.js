...
module.exports = (function () {
  ...
  let sendType1Message = function* (credentials, options) {
    ...
    return this.type1GetAsync(options);
  }
  ...
  let sendType3Message = function* (type1Response, credentials, options) {
    ...
    if (type1Response.headers.location) {
      ...
      return this.type3Request[options.method + "Async"](options);
    }
    ...
    return this.type3Request[method + "Async"](options);
  }
  ...
  let obtainContextInfo = function* (tenantDomain, credentials) {
    ...
    let type1Response = yield sendType1Message.call(this, credentials, options);
    let type3Response = yield sendType3Message.call(this, type1Response, credentials, options);
    ...
    //Update the type3request with the context info
    this.type3Request = this.type3Request.defaults({
      baseUrl: contextInfo.SiteFullUrl,
      headers: {
        "X-RequestDigest": contextInfo.FormDigestValue
      }
    });
    Promise.promisifyAll(this.type3Request);
 ...
 */
  let authenticate = function* (tenantDomain, workstation, domain, username, password) {
    ...
    self.type1GetAsync = Promise.promisify(type1Request.get);
    ...
    Promise.promisifyAll(self.type3Request);
    ...
    let fnMethod = function (options, callback) {
      return Promise.coroutine(function* () {
        ...
        let type1Response = yield sendType1Message.call(self, credentials, options);
        return yield sendType3Message.call(self, type1Response, credentials, options);
    ...
    return {
      ...
      ensureContextInfo: function () {
        self.contextInfoResult = ensureContextInfo.call(self, tenantDomain, credentials, self.contextInfoResult);
        return self.contextInfoResult;
      },
      ntlm: ntlm,
