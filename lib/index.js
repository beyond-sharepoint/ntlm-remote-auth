 ...
 */
  let ensureContextInfo = function* (tenantDomain, credentials, force) {
    if (!force && this.currentContextInfo && moment(this.currentContextInfo.expires).isAfter(moment())) {
      return this.currentContextInfo;
    }

    return this.currentContextInfo = yield Promise.coroutine(obtainContextInfo).call(this, tenantDomain, credentials);
  };
 ...
 */
  let authenticate = function* (tenantDomain, workstation, domain, username, password) {
    ...
    yield Promise.coroutine(ensureContextInfo).call(self, tenantDomain, credentials);

    let fnMethod = function (options, callback) {
      return Promise.coroutine(ensureContextInfo).call(self, tenantDomain, credentials)
        .then(function (contextInfo) {
          ...
          return Promise.promisify(sendType1Message).call(self, credentials, options);
        })
        .then(function (type1Response) {
          return Promise.promisify(sendType3Message).call(self, type1Response, credentials, options);
        })
    ...
    return {
      contextInfo: self.currentContextInfo,
      ensureContextInfo: ensureContextInfo.bind(self, tenantDomain, credentials),
      ensureContextInfoAsync: Promise.coroutine(ensureContextInfo).bind(self, tenantDomain, credentials),
      ntlm: ntlm,
      request: defaultSPRequest,
      requestAsync: Promise.promisify(defaultSPRequest),
      sendType1Message: sendType1Message.bind(self),
      sendType1MessageAsync: Promise.coroutine(sendType1Message).bind(self),
      sendType3Message: sendType3Message.bind(self),
      sendType3MessageAsync: Promise.coroutine(sendType3Message).bind(self),
