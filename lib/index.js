 ...
 */
  let authenticate = function* (tenantDomain, workstation, domain, username, password) {
    ...
    self.contextInfoResult = yield Promise.coroutine(obtainContextInfo).call(self, tenantDomain, credentials);

    let fnMethod = function (options, callback) {
      return Promise.coroutine(ensureContextInfo).call(self, tenantDomain, credentials, self.contextInfoResult)
        .then(function (contextInfoResult) {
          self.contextInfoResult = contextInfoResult;
